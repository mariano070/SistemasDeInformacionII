...
using System.IO;
public class bldhomeintranet : GxBaseBuilder
{
   string cs_path = "." ;
   public bldhomeintranet( ) : base()
   {
   }
   ...
   {
      bldhomeintranet x = new bldhomeintranet() ;
      x.SetMainSourceFile( "homeintranet.cs");
   ...
   {
      ItemCollection sc = new ItemCollection() ;
      sc.Add( @"bin\homeintranet.dll", cs_path + @"\homeintranet.rsp");
      return sc ;
   }
   ...
   public override bool ToBuild( String obj )
   {
      ...
      if ( obj == @"bin\GeneXus.Programs.Common.dll" )
      {
         ...
         if (checkTime(obj, cs_path + @"\type_SdtMenuData_MenuDataItem.cs" ))
            return true;
         if (checkTime(obj, cs_path + @"\type_SdtSDT_Posiciones.cs" ))
            return true;
