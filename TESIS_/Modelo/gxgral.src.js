...
var gx = {
	...
	dom: {
		...
		anchorHasLink: function(anchor){
			if (anchor && anchor.tagName.toLowerCase() == "a" && anchor.href && anchor.href.search(/^javascript:/ig) == -1)
				return true;
			return false;
		},
	...
	evt: {
		...
		onclick: function( evt) {
			gx.evt.userReady = false;
			if (gx.grid.drawAtServer && gx.evt.processing)
			{
		...
		onchange: function(Ctrl) { 	
			...
			if (Ctrl.type == "radio" && Ctrl.onfocus != undefined && (gx.util.browser.isChrome() || gx.util.browser.isWebkit()))
            	           Ctrl.onfocus();
			var vStruct;
			var changed = false;
		    ...
		    if (changed && vStruct && gx.fn.lastMainLevelCtrlId(gx.O.focusControl,vStruct.grid))
			{
				if (Ctrl.onblur)
					Ctrl.onblur();
		...
		invalidDSPKey: function( evt) {
	         var fKey = event.keyCode - 111;
           	 if (evt.shiftKey)
             	   fKey += 12;
          	if (this.keyListeners[fKey][gx.O.CmpContext] == 'ENTER')
                    return true;
                if (evt.keyCode == 13)
                    return true;
		return false;
...
gx.num = {
	...
	extractValue: function(picture, strnum) {
        var pchars=picture.replace(/[\+\-\d,*\.*Z*\s]+/, '');
        var value=strnum;
...
gx.popup = {
	...
	openPrompt: function( PgmName, PgmParms, IsMod, CmpCtx, InMasterPage, IsAuto, EncryptParms) {
		if (gx.popup.currentPrompt == null)
		{
			...
			popup.EncryptParms = EncryptParms;
			popup.open();
		}
	},
	...
	ext: {
		...
		close: function(popupObj) {
			...
			if (iFrame)
				iFrame.src = ''; //avoid iframe reloading FFox
		...
		autofit: function(popupObj, frameDoc, doShow) {		
			...
			if (popupObj.autoresize != 0)
			{
				if (frameDoc && frameDoc.body)
				{
					...
					var hOffset = gx.util.browser.isIE() ? 1 : 0;
					diffW = Math.max(frameDoc.body.scrollWidth, titleEl.clientWidth + 10) - cW - ((dW > 0)?dW:0);
						...
						else
						{
							if (gx.usrPtys[gxCtrl.id] !== undefined )
							{
								...
								for (Pty in gx.usrPtys[gxCtrl.id])
								{
									if (Pty == 'enabled')
										gxCtrl.rtEnabled = true;
									gxCtrl[Pty] = gx.usrPtys[gxCtrl.id][Pty];
								}
							}
						}
...
gx.GxObject = function()
{
	...
	this.getUserFocus = function()
	{
		...
		if (!gx.fn.isValidUserFocus(usrFocusId))
		{
...
gx.fn = {
	...
	getControlValue_impl: function( ControlId, mode) {
		try
		{
			...
			if (Control != null)
			{
				...
				{
					var value = gx.fn.getRadioValue(gx.dom.byName(ControlId));
					if (vStruct && vStruct.type == 'boolean')
						return gx.lang.gxBoolean(value);
					return value;
				}
				if (Control.nodeName == 'SELECT' && Control.options.length > 0)
				{
					if (Control.selectedIndex != -1)
						return (mode == 'screen') ? Control.options[Control.selectedIndex].text : Control.options[Control.selectedIndex].value;
					else
						return (mode == 'screen') ? Control.options[0].text : Control.options[0].value;
				}
    ...
    setEnabled: function(el, Value) {
        if (this.disabledAsSpan(el)) {
            ...
            if (gx.lang.booleanValue(Value)) {
                ...
                if (el.style.display != 'none' && el.type != 'password' && el.type != 'image') {
                    roel = this.getRONode(el.id, true);
                    if (roel) {
                        this.setVisible(el, 0);
                        this.setVisible(roel, 1);
                        this.setControlValue_span_safe(roel, gx.fn.getControlValue( gx.dom.id(el), 'screen') || el.value);
                    }
                }
            }
        }
	...
	isValidUserFocus: function( ControlId) {
		if (ControlId == 'notset')
			return true;
		if (ControlId)
		{
			var Control = gx.dom.byId(ControlId);
			if (Control && gx.fn.isAccepted(Control) && gx.fn.isVisible(Control))
				return true;
		}
		return false;
	},
	...
	lastMainLevelCtrlId: function(CtrlId, GridId) {
	  return (GridId==0 && CtrlId==this.lastCtrlId());
	},
			...
			for (var i=0;i<len;i++)
			{
				...
				if (vStruct != null)
				{
					...
					if (control && gx.fn.isAccepted( control))
						if (mindButtons || (control.type != 'submit' && control.type != 'image' && control.type != 'button'))
							return control;
				}
			}
		}
